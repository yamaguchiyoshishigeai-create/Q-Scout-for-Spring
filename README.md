...
- CLI 起点で分析パイプラインを構成しつつ、Web アプリとして包める拡張性を確保
...
`mvn test` には、既定の日本語ロケールに対する i18n 回帰確認、`?lang=ja` / `?lang=en` の切り替え、ローカライズされた人間向けレポート、違反ゼロ時のレポート文言、英語固定の AI 向け Markdown 契約に関する検証が含まれます。
